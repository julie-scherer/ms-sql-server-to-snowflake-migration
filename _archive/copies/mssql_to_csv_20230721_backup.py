...
def create_bcp_log_file(dir, prefix):
    """
    ...
    """
    make_dir(dir)
    log_path = os.path.join(dir, datetime.now().strftime(f'{prefix}_%Y%m%d_%H%M_Log.txt'))
    return log_path
...
def run_bcp_export(database_name, table_name, csv_path, bcp_query, log_path, err_path):
    """
    ...
    try:
        ...
        logging.info(f"Successfully executed BCP utility for {database_name}.dbo.{table_name}: {csv_path}")
...
def bcp_export_all_records(database_name, table_name, csv_path):
    """
    ...
    bcp_log_file = create_bcp_log_file(os.path.join(logs_dir, 'bcp_log'), f'BCP_{database_name}_{table_name}')
    bcp_err_file = create_bcp_log_file(os.path.join(logs_dir, 'bcp_err'), f'BCP_{database_name}_{table_name}')

    bcp_query = f"""SELECT * FROM {database_name}.[dbo].[{table_name}];"""

    return run_bcp_export(database_name, table_name, csv_path, bcp_query, bcp_log_file, bcp_err_file)


# Export CSVs in batches
def bcp_export_batches(database_name, table_name, csv_path, order_by, offset_row_count, fetch_row_count):
    """
    ...
    Returns:
        str: Path to the exported CSV file.
    """
    logging.info(f"Running BCP command to export batches of records in {database_name}.dbo.{table_name}... ")

    # create logs and error files
    bcp_log_file = create_bcp_log_file(os.path.join(logs_dir, 'bcp_log'), f'BCP_{database_name}_{table_name}_B{offset_row_count}_{fetch_row_count}')
    bcp_err_file = create_bcp_log_file(os.path.join(logs_dir, 'bcp_err'), f'BCP_{database_name}_{table_name}_B{offset_row_count}_{fetch_row_count}')
...
# Main BCP function
def run_bcp_utility(database_name, table_name, src_tbl_dir, csv_filename, cursor):
    """
    ...
    Parameters:
        ...
        csv_filename (str): Base filename for CSV files.
    ...
    try:
        ...
        if num_batches <= 1:
            ...
            return csv_path # exit the function when done
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=batch_workers) as executor:
            ...
            for batch_num in range(1,num_batches+1):
                ...
                offset_row_count = batch_size * batch_num
                fetch_row_count = offset_row_count + batch_size + 1
                print(f"offset row count = {offset_row_count}")
                print(f"fetch row count = {fetch_row_count}")
                
                bcp_export_batches(database_name, table_name, csv_path, order_by, offset_row_count, fetch_row_count)

                future = executor.submit(bcp_export_batches, database_name, table_name, csv_path, order_by, offset_row_count, fetch_row_count)
...
def run_replace_nan_parallel(csv_paths, max_workers=5):
    """
    Run the replace_nan function on each CSV file in parallel.

    Parameters:
        csv_paths (list): List of file paths to CSV files to be processed.
    ...
    def replace_nan(csv_path):
        ...
        try:
            ...
            df = pd.read_csv(
                ...
                na_values=na_values,
                sep=delimiter,
                lineterminator=linebreak,
                header=None,
                encoding=encoding,
                
                # encoding='utf-8',     # Try different encodings here
                # encoding='utf-8-sig',
                # encoding='latin-1',
                # encoding='ISO-8859-1',

                # error_bad_lines=False,  # Skip lines with decoding errors
            )
            ...
            # Save back to CSV
            df.to_csv( csv_path,  index=False, header=False, sep='|', na_rep='NULL' )
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        # Submit tasks for each CSV file to the futures executor
        futures = [executor.submit(replace_nan, csv_path) for csv_path in csv_paths]

        # Wait for the tasks to complete and handle exceptions
        concurrent.futures.wait(futures)

        # Get results and handle exceptions (if any)
        for csv_path, future in zip(csv_paths, futures):
            try:
                future.result()
            except Exception as e:
...
# - - - - - - - - BCP > PIGZ > MOVE - - - - - - - - - -
def run_pipeline(database_name, table_name, cursor):
    ...
    # export data from the database table to CSV files and store them in local dir
    csv_files = run_bcp_utility(database_name, table_name, src_tbl_dir, csv_filename, cursor)
    # process the CSV files present in local dir >> preserves original csv if it encounters an error
    run_replace_nan_parallel(csv_files)
    # run pigz to compress CSV and then move the zip file to the shared drive folder
    ...
    for idx, database in enumerate(databases):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for idx, table_name in enumerate(table_names):
                future = executor.submit(run_pipeline, database_name, table_name, cursor)

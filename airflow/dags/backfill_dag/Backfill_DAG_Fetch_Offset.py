...
def Backfill_DAG():
	...
	def get_runtime_params(mssql_table_name) -> dict:
		...
		file_name = f"{mssql_table_name}_Backfill"
	...
	def export_mssql_batches(runtime_params):
		...
		for i in range(BATCH_START_IDX, num_iterations):
			batch = i+1
			offset = i * BATCH_SIZE
	...
	# # @task
	# def mssql_to_s3(runtime_params, offset=None, batch=None):
	# 	"""
	...
	for mssql_table_name in TABLE_LIST: 
		with TaskGroup(group_id=f"{mssql_table_name}_Task") as backfill_table:
			...
			# end = time.time()
			# exec_time = end - start 
			# logging.info(f"Total time to execute {mssql_table_name} = {exec_time}")

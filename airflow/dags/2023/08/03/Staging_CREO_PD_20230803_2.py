...
def Staging_CREO():
	...
	def get_runtime_params(sf_table_name, ds=None) -> dict:
		...
		get_numbered_cols = get_sf_hook().get_first(f"""
		...
		get_creo_utils = get_sf_hook().get_first(f"""
			SELECT * FROM ARES.ETL.CREO_UTILS
			WHERE table_name = {sf_table_name};
		""")
		logging.info(f"get_creo_utils = {get_creo_utils}")

...
fullTableList = {
    ...
    "CREO_FOLDERCONTACT_HIST": {"sql": None, "key_column": None, "keys": ["CONTACT_KEY", "FOLDER_KEY"], "table_filtered_by": None}, 
...
def Staging_CREO():
    ...
    def get_runtime_params(table_name, ds=None) -> dict:
        ...
        s3_file_path = aod.strftime(f"CREO/%Y/%m/%d/{table_name}")
    ...
    def copy_mssql_to_s3(runtime_params, table):
        ...
        sql_file = fullTableList[table]["sql"]
        table_filtered_by = fullTableList[table]["table_filtered_by"]
        key_column = fullTableList[table]["key_column"]
        ...
        if sql_file != None and table_filtered_by != None:
            ...
            sfsql_query = f"""
                DELETE FROM STG.{table_name}
                WHERE CAST({table_filtered_by} AS DATE) = TO_DATE('{aod}');
            """
        ...
        # - - - - - - - - - - - - - - - - - - - - - - - - 
            ...
            sfsql_query = f"""
                DELETE FROM STG.{table_name}
                WHERE METADATAFILENAME = '{s3_file_path}/{file_name}';
            """
            logging.info(f"Resetting Snowflake table: \n{sfsql_query}")
            sf_hook.run(sfsql_query)
    ...
    @task
    def copy_to_snowflake(runtime_params):
        """

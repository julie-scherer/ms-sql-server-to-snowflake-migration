...
def Staging_CREO():
    ...
    def get_runtime_params(table_name, ds=None) -> dict:
        ...
        yesterday = datetime.strptime(ds, "%Y-%m-%d")
    ...
    def copy_mssql_to_s3(runtime_params, table):
        ...
        ## << #1 Backfill using key value >>
        if (key_column != None) or (select_key_column == True):
            ## If there's no key column, get the first key in the keys list
            if not key_column:
            ...
            if max_key_value == None:
                max_key_value = 0
            logging.info(f"<< Retrieved max key value loaded: {max_key_value} >>")
            ...
            ## Starting at the max key value, read data from MSSQL
            if sql_file == None:
                mssql_query = f"""
                    SELECT * FROM [dbo].[{trunc_table_name}] WHERE [{key_column}] >= {max_key_value};
                """

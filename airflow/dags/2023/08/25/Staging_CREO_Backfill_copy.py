...
fullTableList = {
    # >> #1 Custom SQL Loads 
    "CREO_MESSAGE_HIST": {"sql": "COPY_CREO_MESSAGE_BACKFILL.sql", "key_column": "MESSAGE_KEY", "keys": ["CONTAINER_KEY", "TEMPLATE_KEY", "SEND_AFTER_MESSAGE_KEY", "DELIVERY_STATUS_KEY", "IN_REPLY_TO_MESSAGE_KEY", "COMMUNICATION_MAILING_KEY", "MESSAGE_KEY", "DATASET_ROW_KEY"], "table_filtered_by": "DATE_ENTERED"}, 
    # << #1 Custom SQL Loads 
    ...
    # ## >> #4 Partial full load using MAX key (highest cardinality)
    # # \/ Tables with composite PKs joined with tables that have DATE column \/
    "CREO_DATASETCELL_HIST": {"sql": None, "key_column": None, "keys": ["DATASET_VALUE_KEY", "DATASET_ROW_KEY", "DATASET_COLUMN_KEY"], "table_filtered_by": None},
    "CREO_PACKAGETEMPLATE_HIST": {"sql": None, "key_column": None, "keys": ["PACKAGE_KEY", "TEMPLATE_KEY"], "table_filtered_by": None}, 
    "CREO_FOLDERCONTACT_HIST": {"sql": None, "key_column": None, "keys": ["FOLDER_KEY", "CONTACT_KEY",], "table_filtered_by": None}, 
    "CREO_FOLDERMESSAGE_HIST": {"sql": None, "key_column": None, "keys": ["MESSAGE_KEY", "FOLDER_KEY"], "table_filtered_by": None}, 
    "CREO_TEMPLATERULE_HIST": {"sql": None, "key_column": None, "keys": ["TEMPLATE_KEY", "RULE_KEY"], "table_filtered_by": None}, 
...
def Staging_CREO_Backfill():
    ...
    def get_runtime_params(table_name, ds=None) -> dict:
        ...
        s3_file_path = f"CREO/Backfill/Staging/{trunc_table_name}"
        file_name = yesterday.strftime(f"{trunc_table_name}_%Y%m%d")
    ...
    def copy_mssql_to_s3(runtime_params, table):
        ...
        if sql_file != None and table_filtered_by != None:
            ## Get the most recent date loaded in Snowflake
            snowflake_max_aod = f"""
            ...
            if max_aod == None:
                max_aod = 0
            logging.info(f"MAX(asOfDate) = {max_aod}")

            ## Reset Snowflake table
            reset_sfsql_query = f"""
                DELETE FROM STG.{table_name} WHERE CAST({table_filtered_by} AS DATE) = TO_DATE('{yesterday}');
            """
            logging.info(f"Resetting Snowflake table: \n{reset_sfsql_query}")
            sf_hook.run(reset_sfsql_query)

            ## Read in MSSQL data that came in after the last load date and before the current date
            logging.info(f"<< Backfill using custom SQL query and `{table_filtered_by}` >>")
            with open(f"{staging_dir}/{sql_file}", "r") as file:
                mssql_query = file.read().format(
                    lastLoadDate=max_aod,
                    yesterday=yesterday,
                )
        ...
        # - - - - - - - - - - - - - - - - - - - - - - - - 
            ...
            if max_aod == None:
                max_aod = '1900-01-01'
            logging.info(f"MAX({key_column}) = {max_aod}")
            ...
            mssql_query = f"""
                ...
                WHERE CAST([{table_filtered_by}] AS DATE) > '{max_aod}'
                AND CAST([{table_filtered_by}] AS DATE) <= '{yesterday}';
            """
            logging.info(f"Getting results from MSSQL: \n{mssql_query}")
            df = mssql_hook.get_pandas_df(mssql_query)
        
        ## << #3 Partial full load with max key >>
        # If there's a key column to filter by...
        # - - - - - - - - - - - - - - - - - - - - - - - - 
        elif key_column != None or keys != None:
            ## If there's no key column, get the max value for the first key in keys 
            # (note the key array is ordered from highest to lowest cardinality)
            if not key_column:
                keys = fullTableList[table]["keys"]
                key_column = keys[0]
            logging.info(f"The key column for {trunc_table_name.lower()} is `{key_column}`")

            ## Get the max value for the key column
            logging.info(f"<< Backfill using MAX key `{key_column}` >>")
            snowflake_max = f"""
            ...
            for idx, chunk in enumerate(chunks):
                ...
                s3_hook.load_bytes(
                    ...
                    key=f"inbound/{s3_file_path}/{chunk_num}_{file_name}",
                    replace=True,
                )
                logging.info(f'<< Uploading chunk to S3: {chunk_num}_{file_name} >>')
...
creo_run = Staging_CREO_Backfill()

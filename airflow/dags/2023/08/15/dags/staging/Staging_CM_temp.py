...
def get_table_utils(table_name):
    utils_query = f"""
            SELECT * FROM ETL.CM_UTILS
            WHERE table_name ILIKE 'CM_{table_name}_HIST'
        """
...
@dag(
    ...
    doc_md=__doc__,
    template_searchpath=f"include/sql/Staging_CM/",
...
def Staging_DAG():
    ...
    def get_runtime_params(table_name, ds=None) -> dict:
        ...
        """
        get_numbered_columns_query = f"SELECT ETL.COPYSELECT('STG','CM_{table_name}_HIST',3)"
        ...
        s3_dir_path = aod.strftime(f"CM/%Y/%m/%d/{table_name}")
    ...
    def reset_snowflake(runtime_params):
        ...
        if sql_file and sql_file != 'NULL':
            sfsql_query = f"""
                DELETE FROM STG.CM_{table_name}_HIST 
        ...
        if table_filtered_by and table_filtered_by != 'NULL':
            ...
            sfsql_query = f"""
                    DELETE FROM STG.CM_{table_name}_HIST 
                    WHERE FILENAME = 'inbound/{s3_path}';
                """
    ...
    def copy_s3_to_snowflake(runtime_params):
        ...
        sql_query = f"""
            -- \\ CM_{table_name}_HIST
            COPY INTO STG.CM_{table_name}_HIST 
            FROM (
                SELECT 
    ...
    def write_to_dataset():
        ...
        with open(dataset_file, "a") as file:
            file.write("Staging_CM data load complete")
    ...
    for table_name in TABLE_LIST: 
        with TaskGroup(group_id=f"CM_{table_name}_HIST_Task") as staging_pipeline:

...
def get_table_utils(table_name):
    utils_query = f"""
            SELECT * FROM ETL.LD_UTILS
            WHERE table_name ILIKE 'LD_{table_name}_HIST'
        """
...
def Staging_DAG():
    ...
    def get_runtime_params(table_name, ds=None) -> dict:
        ...
        """
        get_numbered_columns_query = f"SELECT ETL.COPYSELECT('STG','LD_{table_name}_HIST',3)"
    ...
    def write_to_dataset():
        ...
        with open(dataset_file, "a") as file:
            file.write("Staging_LD data load complete")

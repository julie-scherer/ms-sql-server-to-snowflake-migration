...
def Staging_DAG():
    ...
    def mssql_to_s3(runtime_params):
        ...
        """
        logging.info(f"<< Exporting table data: {runtime_params} >>")
        ...
        ## Assign the df and query to empty value for error catching later
        ...
        if sql_file and sql_file != 'NULL':
            logging.info(f"<< #1 Running Custom SQL: {table_name} >>")
        ...
        # If there's a date column to filter by...
            ...
            logging.info(f"<< #2 Running table_filtered_by = {table_filtered_by}: {table_name} >>")
        ...
        # If there's a key column to filter by...
            ...
            logging.info(f"<< #4 Running key_column = {key_column}: {table_name} >>")
        ...
        # If none of the above are true...
            ...
            logging.info(f"<< #3 Running full load: {table_name} >> ")
            ...
            for idx, chunk in enumerate(chunks):
                dfs.append(chunk)
                logging.info(f'<< Adding chunk {file_name}_{idx+1} to list >>')
            df = pd.concat(dfs)
                ...
                #     replace=True,
                # )
                # logging.info(f'<< Uploading chunk to S3: {file_name}_{idx}.czv.gz >>')
        ...
        if df.empty:
            ...
            logging.info(f'<< Uploading df to S3: {zip_file_name} >>')

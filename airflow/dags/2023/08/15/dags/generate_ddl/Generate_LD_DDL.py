...
def Generate_DDL_DAG():
    ...
    def create_temp_table():
        create_pretty_ddl_table = f"""
            CREATE OR REPLACE TABLE ETL.LD_DDL_TEMP (
    ...
    def raw_mssql_to_s3():
        ...
        get_s3_hook().load_bytes(
            ...
            key=f"inbound/LD/DDL/LD_DDL.csv",
            replace=True,
        )

    @task
    def load_s3_to_sf_temp_table():
        get_sf_hook().run(f"TRUNCATE TABLE IF EXISTS ETL.LD_DDL_TEMP")
        sql_query = f"""
        COPY INTO ETL.LD_DDL_TEMP
        FROM (
            SELECT 
                $1, $2
            FROM @ETL.INBOUND/LD/DDL/
        )
        FILE_FORMAT = ( 
            {FILE_FORMAT}
        ) 
    ...
    def create_pretty_ddl_table():
        query = f"""
        CREATE OR REPLACE TABLE ETL.LD_DDL (
            table_name VARCHAR,
            column_name VARCHAR,
            data_type VARCHAR,
            collation VARCHAR,
    ...
    def delete_temp_table():
        query = f"""
        DROP TABLE IF EXISTS ETL.LD_DDL_TEMP;
        """
        get_sf_hook().run(query)

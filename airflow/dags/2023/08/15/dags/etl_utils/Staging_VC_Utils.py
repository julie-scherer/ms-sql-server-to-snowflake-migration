...
def Staging_Utils():
    ...
    def get_staging_utils() -> dict:
        ...
        ddl_table = get_sf_hook().get_pandas_df(f"SELECT * FROM ETL.VC_DDL")
        ...
        for table_name in table_names:
            ...
            custom_sql_dir = "/usr/local/airflow/include/sql/Staging_VC"
            custom_sql_file_name = f"COPY_VC_{table_name.upper()}.sql"
            ...
            mssql_table_name = table_name.replace(f'VC_', '').replace('_HIST', '')
            ...
            SNOWFLAKE_TABLE = f"VC_{table_name.upper()}_HIST"
    ...
    def create_snowflake_table():
        # get_sf_hook().run(f"DROP TABLE IF EXISTS ETL.VC_UTILS")
        
        # Create Snowflake table
        query = f"""
        CREATE OR REPLACE TABLE ETL.VC_UTILS (
    ...
    def df_to_s3(fullTableList):
        ...
        s3_path=f"inbound/VC/UTILS/{CSV_FILE_NAME}"
    ...
    @task
    def s3_to_snowflake():
        get_sf_hook().run(f"TRUNCATE TABLE IF EXISTS ETL.VC_UTILS")
        sql_query = f"""
            COPY INTO ETL.VC_UTILS
            FROM (
                SELECT $1, $2, $3, $4, $5
                FROM @ETL.INBOUND/VC/UTILS/
            )
            FILE_FORMAT = ( 
                {FILE_FORMAT}
            ) 

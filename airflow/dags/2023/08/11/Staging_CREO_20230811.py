...
def Staging_CREO():
    ...
    def reset_snowflake(runtime_params):
        ...
        if sql_file and sql_file != 'NULL':
            # sfsql_query = use_sql_file(aod=aod, table_name=table_name)
            sfsql_query = f"""
                DELETE FROM STG.{DATABASE_NAME}_{table_name}_HIST 
                WHERE CAST(DATE_ENTERED AS DATE) = TO_DATE('{aod}');
            """
        ...
        if table_filtered_by and table_filtered_by != 'NULL':
            # sfsql_query = use_date_column(aod=aod, table_name=table_name)
            sfsql_query = f"""
                DELETE FROM STG.{DATABASE_NAME}_{table_name}_HIST 
                WHERE ASOFDATE = TO_DATE('{aod}');
            """
            ...
            # sfsql_query = use_key_column(table_name=table_name, s3_path=s3_path)
            sfsql_query = f"""
                    DELETE FROM STG.{DATABASE_NAME}_{table_name}_HIST 
                    WHERE METADATAFILENAME = 'inbound/{s3_path}';
                """
            ...
            # sfsql_query = no_sql_date_keycol(table_name=table_name, aod=aod)
            sfsql_query = f"""
                DELETE FROM STG.{DATABASE_NAME}_{table_name}_HIST 
                WHERE ASOFDATE = TO_DATE('{aod}');
            """

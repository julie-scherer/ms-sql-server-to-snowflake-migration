...
dataset_file = "include/datasets/CREO_Backfill_Dataset.txt"
dataset_obj = Dataset(dataset_file)

fullTableList = {
    # >> #1 Custom SQL Loads 
    "CREO_MESSAGE_HIST": {"sql": "COPY_CREO_MESSAGE.sql", "key_column": "MESSAGE_KEY", "keys": ["CONTAINER_KEY", "TEMPLATE_KEY", "SEND_AFTER_MESSAGE_KEY", "DELIVERY_STATUS_KEY", "IN_REPLY_TO_MESSAGE_KEY", "COMMUNICATION_MAILING_KEY", "MESSAGE_KEY", "DATASET_ROW_KEY"], "table_filtered_by": "DATE_ENTERED"}, # << Custom MSSQL query to read data from MSSQL 
    # << #1 Custom SQL Loads 
    ...
    "CREO_TEMPLATE_HIST": {"sql": None, "key_column": "TEMPLATE_KEY", "keys": ["TEMPLATE_KEY", "BASE_TEMPLATE_KEY"], "table_filtered_by": "DATE_ENTERED",  "copy_into": "COPY_CREO_TEMPLATE.sql"}, # << Custom query to copy data into SF
    ...
    "CREO_MESSAGEPART_HIST": {"sql": None, "key_column": "MESSAGE_PART_KEY", "keys": ["MESSAGE_PART_KEY", "MESSAGE_KEY"], "table_filtered_by": None,  "copy_into": "COPY_CREO_MESSAGEPART.sql"}, # << Custom query to copy data into SF
    "CREO_MESSAGEPARTV2_HIST": { "sql": None,  "key_column": "MESSAGE_PART_KEY",  "keys": [ "MESSAGE_PART_KEY",  "MESSAGE_KEY" ],  "table_filtered_by": None ,  "copy_into": "COPY_CREO_MESSAGEPART.sql"}, # << Custom query to copy data into SF
...
@dag(
    ...
    catchup=False,
    schedule=None,  # 6AM EST
    dagrun_timeout=timedelta(hours=4),
    doc_md=__doc__,
    tags=["Author: Julie Scherer","Staging_CREO"],
...
def CREO_Backfill():
    ...
    def get_runtime_params(table_name, ds=None) -> dict:
        ...
        s3_file_path = yesterday.strftime(f"CREO/Backfill/%Y/%m/%d/{trunc_table_name}")
        file_name = yesterday.strftime(f"{trunc_table_name}_%Y%m%d.csv")
    ...
    def copy_mssql_to_s3(runtime_params, table):
        ...
        yesterday = runtime_params.get("yesterday")
        ...
        ## << Backfill using DATE column >>
        # - - - - - - - - - - - - - - - - - -
        if table_filtered_by != None:
            logging.info(f"<< Backfill using `{table_filtered_by}` >>")

            ## Get the date of the most recent record loaded into Snowflake
            snowflake_max_date = f"""
                SELECT MAX({table_filtered_by}) FROM STG.{table_name};
            """
            max_date_df = sf_hook.get_pandas_df(snowflake_max_date)
            max_date = max_date_df.iloc[0, 0]
            if max_date == None:
                max_date = '1900-01-01' #equivalent to 0 but in date format
            logging.info(f"Retrieved max date for {table_filtered_by}: {max_date}")

            ## Delete records in Snowflake with that date
            reset_sfsql_query = f"""
                DELETE FROM STG.{table_name} WHERE TO_DATE({table_filtered_by}) = TO_DATE('{max_date}');
            """
            ...
            mssql_query += f"""
            WHERE CAST([{table_filtered_by}] AS DATE) >= CAST('{max_date}' AS DATE) 
            AND CAST([{table_filtered_by}] AS DATE) < CAST('{yesterday}' AS DATE)
            """
            
            logging.info(f"Getting results from MSSQL: \n{mssql_query}")
            chunks = mssql_hook.get_pandas_df_by_chunks(sql=mssql_query, chunksize=20000)
        
        ## << Backfill using KEY column >>
        # - - - - - - - - - - - - - - - - -
        elif (key_column != None) or (select_key_column == True):
            ## If there's no PK column, get the first key from the keys list only if select_key_column is True, 
            ## meaning we found a select key out of several PK/FKs that can be used to identify unique records
            if not key_column:
                keys = fullTableList[table]["keys"]
                key_column = keys[0]
            
            logging.info(f"<< Backfill using `{key_column}` >>")
            ...
            max_key_value = max_value_df.iloc[0, 0]
            if max_key_value == None:
                max_key_value = 0
            logging.info(f"Retrieved max value for {key_column}: {max_key_value}")

            ## Delete records in Snowflake with that key value
            reset_sfsql_query = f"""
                DELETE FROM STG.{table_name} WHERE {key_column} = {max_key_value};
            """
            logging.info(f"Resetting Snowflake table: \n{reset_sfsql_query}")
            sf_hook.run(reset_sfsql_query)
        ...
        ## << #2 Backfill using asOfDate >>
        else:
            ## Get the most recent asOfDate (latest date) in Snowflake
            snowflake_max_aod = f"""
                SELECT MAX(ASOFDATE) FROM STG.{table_name};
            """
            max_aod_df = sf_hook.get_pandas_df(snowflake_max_aod)
            max_aod = max_aod_df.iloc[0, 0]
            if max_aod == None:
                max_aod = yesterday
            logging.info(f"<< Retrieved last date loaded: {max_aod} >>")

            ## Reset Snowflake table
            reset_sfsql_query = f"""
                DELETE FROM STG.{table_name} WHERE ASOFDATE = TO_DATE('{max_aod}');
            """
            logging.info(f"Resetting Snowflake table: \n{reset_sfsql_query}")
            sf_hook.run(reset_sfsql_query)
        ...
        if chunks != None:
            for idx, chunk in enumerate(chunks):
                ...
                logging.info(f'Uploaded chunk #{chunk_idx} to S3: inbound/{s3_file_path}/{chunk_idx}_{file_name} >>')

            logging.info(f'<< Finished uploading chunks to S3 >>')

        else:
            logging.info('!!! No results from MSSQL !!!')
    ...
    def write_to_dataset():
        ...
        with open(dataset_file, "a") as file:
            file.write("CREO_Backfill data load complete")

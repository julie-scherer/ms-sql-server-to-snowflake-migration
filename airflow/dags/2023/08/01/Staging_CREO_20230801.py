...
@dag(
	...
	schedule_interval='@once',  # ! testing
...
def Staging_CREO():
	...
	def get_mssql_query(**context):
		...
		runtime_params = context["ti"].xcom_pull(task_ids="get_runtime_params", key="return_value")
	...
	def run_bcp(**context):
		...
		query = context["ti"].xcom_pull(task_ids="get_mssql_query", key="return_value")
		runtime_params = context["ti"].xcom_pull(task_ids="get_runtime_params", key="return_value")
		# trunc_table_name = context["ti"].xcom_pull(task_ids="get_runtime_params", key="trunc_table_name")
	...
	def run_pigz(**context): 
		...
		local_output_dir = context["ti"].xcom_pull(task_ids="run_bcp", key="return_value")
	...
	def load_zip_to_s3(**context):
		...
		# local_zip_path = context["ti"].xcom_pull(task_ids="run_pigz", key="local_zip_path")
		local_zip_path = context["ti"].xcom_pull(task_ids="run_pigz", key="return_value")
	...
	for sf_table_name in fullTableList: 
		with TaskGroup(group_id=f"Staging_{sf_table_name}") as export_mssql_load_snowflake:
			...
			get_runtime_params_task >> get_mssql_query_task >> run_bcp_task >> run_pigz_task >> load_zip_to_s3_task >> copy_s3_to_snowflake_task

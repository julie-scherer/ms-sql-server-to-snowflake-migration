...
def get_table_utils(table_name):
    utils_query = f"""
            SELECT * FROM ARES.ETL.CM_UTILS
            WHERE table_name = 'CM_{table_name}_HIST'
        """
...
def Staging_DAG():
    ...
    def get_runtime_params(table_name, ds=None) -> dict:
        ...
        query_output = get_sf_hook().get_first(f"""SELECT ETL.COPYSELECT('STG','CM_{table_name}_HIST',3)""")
        columns = query_output[f"ETL.COPYSELECT('STG','CM_{table_name}_HIST',3)"]

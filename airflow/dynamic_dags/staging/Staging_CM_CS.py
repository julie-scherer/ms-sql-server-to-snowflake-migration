"""
### Staging CM DAG(For the Customer Scoring DB)
...
Description..: As there is only 1 table in Customer Scoring DB which we need to load (AgencyData), this DAG will focus on that table only. <br>
                ...
                In addition, this dag will be data driven scheduled so once it completes, it will update the file ds_staging_CM_customer_scoring.txt <br>
Working......: The file ds_staging_CM_customer_scoring.txt should be updated with a new line of text <br>
...
db_name = "awsmssql_cansql_customerscoring_conn"
snowflake = "snowflake_default"

staging_cm_cs_dataset = "include/datasets/ds_staging_CM_CS.txt"
ds_Staging_CM_cs = Dataset(staging_cm_cs_dataset)

fullTableList = {'CM_CS_AgencyData_HIST':{"has_date_entered":True, "sql":None}}
...
def Staging_Customer_Scoring_CM():
    ...
    def get_run_param(table_name,ds=None,ti=None) -> dict:
        ...
        pattern = "CM/CustomerScoring/{year}/{month}/{day}/{table}/".format(year=aod.strftime("%Y"), month=aod.strftime("%m"), day=aod.strftime("%d"), table=table_name.replace("CM_CS_","")[:-5])
        target_file = "{table_t}_{date}.csv.gz".format(table_t = table_name.replace("CM_CS_","")[:-5], date=aod.strftime('%Y%m%d'))
        return {
    ...
    def copy_mssql_to_s3(table_data: dict, table):
        ...
        ## << #2 Table has DATE_ENTERED and no Custom SQL >>
            ...
            #Getting the MSSQL data
            df = mssql_hook.get_pandas_df(sql="select * FROM dbo.{t} WHERE CAST(DATE_ENTERED AS DATE) = '{target_date}';".format(t=table.replace("CM_CS_","")[:-5], target_date = table_data["asOfDate"]))
        else:
            ## << #3 Full Load >>
            if fullTableList[table]["key_column"] == None:
                ...
                chunks_cnt = 0                 
                for chunk in mssql_hook.get_pandas_df_by_chunks(sql="select * FROM dbo.{t};".format(t=table.replace("CM_CS_","")[:-5]),chunksize=20000): #20k rows 
                ...
                df = mssql_hook.get_pandas_df(sql="select * FROM dbo.{table} where {kc} > {mv};".format(table = snowflake_table.replace("CM_CS_","")[:-5], kc = key, mv = max_value))
    ...
    def copy_to_snowflake(table_details: dict):
        ...
        sql_updated = sql_general.format(table_name=table_t, columns=cols, file_format="STG.CM_CSV_PIPE_SH1_EON_GZ",s3_file_path=table_details["s3_file_path"], previous = previous_day, file_pattern = table_details["file_pattern"])
        sf_hook.run(sql_updated)

    @task(outlets=[ds_Staging_CM_cs])
    def write_to_dataset():
        f = open(staging_cm_cs_dataset, "a")
        f.write("cm data load complete for customer scoring db")
        f.close()    
...
cm_run = Staging_Customer_Scoring_CM()
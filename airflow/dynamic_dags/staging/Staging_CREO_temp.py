...
def get_table_utils(table_name):
    utils_query = f"""
            SELECT * FROM ARES.ETL.CREO_UTILS
            WHERE table_name = 'CREO_{table_name}_HIST'
        """
...
def Staging_DAG():
    ...
    def get_runtime_params(table_name, ds=None) -> dict:
        ...
        query_output = get_sf_hook().get_first(f"""SELECT ETL.COPYSELECT('STG','CREO_{table_name}_HIST',3)""")
        columns = query_output[f"ETL.COPYSELECT('STG','CREO_{table_name}_HIST',3)"]

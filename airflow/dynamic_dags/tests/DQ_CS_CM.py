"""
### CM DQ checks 
Triggered by.: STAGING_CM_CS DAG.
Description..: Data Quality checks for nightly loads.
Working......: Checks are made against Source database Customer Scoring
               DAG dynamically creates checks_CM_CS.yml for CM_AgencyData_HIST. And calls SODA passing checks_CM_CS.yml. 
...
fullTableList = {'CM_CS_AgencyData_HIST'}

with DAG(
    dag_id="DQ_CM_CS",
    catchup=False,
    start_date=pendulum.datetime(2023, 7, 18, tz='US/Eastern'),
    schedule = [ds_Staging_CM_CS],
    #schedule_interval = None, # DAG Time Zone set to EST 
    doc_md=__doc__,
    tags=["Author: Shunjian Wang","cross-DAG dependencies", "Staging_CM_CS"],    
    default_args={
        "retries": 1,
    ...
    def set_config(ds = None, **kwargs):
        ...
        file2 = open(f"{SODA_PATH}/checks/checks_CM_CS.yml","a")
        ...
        for table in fullTableList:
            ...
            df = mssql_hook.get_pandas_df(sql="select count(*) FROM dbo.{t} WHERE CAST(DATE_ENTERED AS DATE) = '{target_date}';".format(t=table_name.replace("CM_CS_","")[:-5], target_date = asOfDate))
    ...
    def check_config_exists(**kwargs):
        checks_file_name = f"{SODA_PATH}/checks/checks_CM_CS.yml"
    ...
    remove_checks = BashOperator(
        ...
        bash_command='rm -f /usr/local/airflow/include/soda/checks/checks_CM_CS.yml',
    )

    create_checks = BashOperator(
        task_id='create_checks',
        bash_command='touch /usr/local/airflow/include/soda/checks/checks_CM_CS.yml',
    )
    ...
    @task(outlets=[ds_DQ_CS_CM_DAG])
    def write_to_dataset():
        f = open(dq_cm_cs_ds_name, "a")
        f.write("cm dq check complete for customer scoring db")
        f.close()

    #Operator for running the soda scan
    soda_scan = BashOperator(
        task_id="dq_CM_CS",
        bash_command=f"soda scan -d ds -c {SODA_PATH}/config/configuration_parametrized_CM.yml {SODA_PATH}/checks/checks_CM_CS.yml"
    )
